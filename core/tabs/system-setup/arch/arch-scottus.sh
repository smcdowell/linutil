...
if [ ! -f /usr/bin/pacstrap ]; then
    printf "%b\n" "${RED}This script must be run from an Arch Linux ISO environment.${RC}"
    exit 1
fi

root_check() {
    if [[ "$(id -u)" != "0" ]]; then
        printf "%b\n" "${RED}ERROR! This script must be run under the 'root' user!${RC}"
        exit 0
    fi
}
...
docker_check() {
    if awk -F/ '$2 == "docker"' /proc/self/cgroup | read -r; then
        printf "%b\n" "${RED}ERROR! Docker container is not supported (at the moment)${RC}"
        exit 0
...
arch_check() {
    if [[ ! -e /etc/arch-release ]]; then
        printf "%b\n" "${RED}ERROR! This script must be run in Arch Linux!${RC}"
        exit 0
    fi
}
...
pacman_check() {
    if [[ -f /var/lib/pacman/db.lck ]]; then
        printf "%b\n" "${RED}ERROR! Pacman is blocked.${RC}"
        printf "%b\n" "${RED}If not running remove /var/lib/pacman/db.lck.${RC}"
        exit 0
    fi
}
...
select_option() {
    ...
    while true; do
        ...
        if [ $last_selected -eq -1 ]; then
            printf "%b\n" "${GREEN}Please select an option using the arrow keys and Enter:${RC}"
        fi
        for i in "${!options[@]}"; do
...
filesystem () {
    printf "%b\n" "${GREEN}Please Select your file system for both boot and root${RC}"
    "
...
timezone () {
    ...
    case ${options[$?]} in
        y|Y|yes|Yes|YES)
        printf "%b\n" "${GREEN}${time_zone} set as timezone${RC}"
        export TIMEZONE=$time_zone;;
        n|N|no|NO|No)
        printf "%b\n" "${GREEN}Please enter your desired timezone e.g. Europe/London :${RC}" 
        read -r new_timezone
        printf "%b\n" "${GREEN}${new_timezone} set as timezone${RC}"
        export TIMEZONE=$new_timezone;;
        *) printf "%b\n" "${RED}Wrong option. Try again${RC}";timezone;;
    esac
}
# @description Set user's keyboard mapping. 
keymap () {
    printf "%b\n" "${GREEN}Please select key board layout from this list${RC}"
    ...
    printf "%b\n" "${GREEN}Your key boards layout: ${keymap}${RC}"
...
drivessd () {
    ...
    case ${options[$?]} in
        ...
        *) printf "%b\n" "${RED}Wrong option. Try again${RC}";drivessd;;
    esac
}
